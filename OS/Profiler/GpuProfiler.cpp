...
void addGpuProfiler(Renderer* pRenderer, Queue* pQueue, GpuProfiler** ppGpuProfiler, const char* pName)
{
	...
	{
		vk_addQueryPool(pRenderer, &queryHeapDesc, &pGpuProfiler->pQueryPool[i]);
	...
	vk_getTimestampFrequency(pQueue, &pGpuProfiler->mGpuTimeStampFrequency);
...
void removeGpuProfiler(struct GpuProfiler* pGpuProfiler)
{
	for (uint32_t i = 0; i < GpuProfiler::NUM_OF_FRAMES; ++i)
	{
		...
		vk_removeQueryPool(pGpuProfiler->pRenderer, pGpuProfiler->pQueryPool[i]);
	}
	...
	bool isRoot = false)
{
...
#endif
	{
		...
		vk_cmdBeginQuery(pCmd, pGpuProfiler->pQueryPool[pGpuProfiler->mBufferIndex], &desc);
	}

	if (addMarker)
	{
		vk_cmdBeginDebugMarker(pCmd, color.getX(), color.getY(), color.getZ(), pName);
	}
...
void cmdEndGpuTimestampQuery(Cmd* pCmd, struct GpuProfiler* pGpuProfiler, bool isRoot = false)
{
...
#endif
	{
		...
		vk_cmdEndQuery(pCmd, pGpuProfiler->pQueryPool[pGpuProfiler->mBufferIndex], &desc);
	}

	if (pGpuProfiler->pCurrentNode->mDebugMarker)
	{
		vk_cmdEndDebugMarker(pCmd);
	}
...
void cmdBeginGpuFrameProfile(Cmd* pCmd, ProfileToken nProfileToken, bool bUseMarker)
{
	...
	{
		for (uint32_t i = 0; i < GpuProfiler::NUM_OF_FRAMES; ++i)
			vk_cmdResetQueryPool(pCmd, pGpuProfiler->pQueryPool[i], 0, pGpuProfiler->pQueryPool[i]->mCount);
	...
	vk_cmdResetQueryPool(pCmd, pGpuProfiler->pQueryPool[pGpuProfiler->mBufferIndex], 0, pGpuProfiler->mMaxTimerCount * 2);

	uint32_t nextIndex = (pGpuProfiler->mBufferIndex + 1) % GpuProfiler::NUM_OF_FRAMES;
	pGpuProfiler->mBufferIndex = nextIndex;

	vk_cmdResetQueryPool(pCmd, pGpuProfiler->pQueryPool[pGpuProfiler->mBufferIndex], 0, pGpuProfiler->mCurrentTimerCount * 2);
